...
1. Nachmessung in zwei bis drei Wochen mit `context_fill_analysis.py`: Anteil der Läufe über 200K (Basis 21 Prozent, 59 Prozent des Input-Verbrauchs), Anteil wiederholter Reads (13,9 Prozent), Voll-Read-Anteil (74 Prozent), Reads von `index.md` (55 in 60 Tagen). Dazu das TTL-Experiment im wlh-Projekt ab 04.10. Aufruf und Kennzahlen: `messung/README.md`. Seit 21.09. läuft im Projekt `wlh-preiskalkulation-app` zusätzlich das Experiment Code-Regeln: die sieben Policies aus Abschnitt 4 der Einschätzung (ESLint `max-lines`, `max-lines-per-function`, `complexity`; jscpd; knip; tsc mit `checkJs`; dependency-cruiser; Prettier), Modus je Werkzeug und Basislinie in `docs/code-regeln-basislinie-2026-09-21.md` des wlh-Repos. Die Nachmessung weist das wlh-Projekt deshalb getrennt aus und stellt den Token-Zahlen die Verstoßzahlen aus `npm run pruefe:code:bericht` gegenüber. Die Einschätzung hatte diese Regeln als Token-Maßnahme mit „Nein“ oder „Optional“ bewertet; das Experiment prüft dieses Urteil.
