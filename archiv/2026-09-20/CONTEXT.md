...
**Executor**:
Subagent, der ein gebrieftes Arbeitspaket umsetzt; dahinter steht der Agent `executor`, dessen Modell beim Aufruf nach der Modelltabelle in CLAUDE.md gesetzt wird.
_Avoid_: Worker, Coder-Agent

**Verifier**:
Subagent mit frischem Kontext, der ein Ergebnis gegen Spezifikation bzw. Akzeptanzkriterien prüft und alles meldet (mit Confidence und Severity); dahinter steht der Agent `verifier`, dessen Modell beim Aufruf nach der Modelltabelle in CLAUDE.md gesetzt wird. Das finale Urteil bleibt beim Hauptmodell. Ein Fable-Verifier teilt das Kontingent mit der Hauptsession, deshalb sparsam.
...
**Traceability-Datei**:
Pro Coding-Projekt eine `TRACEABILITY.md` im Repo-Root, die je Ticket-ID in einer Zeile festhält, in welchen Codeteilen das Ticket umgesetzt wurde, dazu ein Halbsatz zum Was.
...
**Pointer**:
Einzeiliger Verweis auf ausgelagertes Material (Skill, Datei), das nur bei Bedarf in den Kontext geladen wird. `~/.claude/rules/*.md` sind kein Pointer-Ziel, sondern Dauerlast: ohne `paths`-Frontmatter in jedem Turn geladen, in Hauptsession und Subagents.
_Avoid_: Inline-Runbook, einkopierter Styleguide

**Agent-Definition**:
Markdown-Datei unter `~/.claude/agents/`, deren Frontmatter den Agenten benennt (Name, Beschreibung, Default-Modell, Tool-Grenzen) und deren Body sein Systemprompt wird. Sie wird nur beim Lauf dieses Agenten geladen und ist damit der Wohnort für Regeln, die nur Ausführende brauchen. Der Body ersetzt den Claude-Code-Standardprompt des Subagenten, aber nicht vollständig: Identitätszeile, Sicherheitsteil und die CLAUDE.md-Hierarchie einschließlich `rules/*.md` und der Skill-Liste bleiben erhalten (gemessen 2026-09-13); verloren gehen der eingebaute Hinweis auf parallele Tool-Calls und der Hinweis, dass ein Subagent keine Chat-Zustimmung einholen kann. Beide Definitionen führen deshalb die Absätze „Arbeitsweise“ und „Grenzen“ wortgleich selbst — eine bewusste Dopplung, die beim Pruning bleibt.
